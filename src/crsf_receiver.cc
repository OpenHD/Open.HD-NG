...
/*
void crsf_battery_packet(std::vector<uint8_t> &buf, float v, float c, uint32_t cap, float remain) {
  buf.clear();
...
int main(int argc, char **argv) {
  ...
  config.add_options()
    ...
    ("port,p", po::value<uint16_t>(&port)->default_value(14550),
